...
export default {
  ...
  methods: {
    ...
    look(row) {
      ...
      baseRequest('/taskLog/getLogInfo', { taskFlowId: this.paramData.taskFlowId, 'subTaskId': row.subTaskId, 'taskNodeId': this.paramData.taskNodeId }).then(response => {
        this.tableData = response.data.item
      })
    },
    reSubTask(row) {
      baseRequest('/taskNodeSub/update', { 'subTaskId': row.subTaskId, 'calStatus': '2' }).then(response => {
        this.$message({
          type: 'success',
          message: '操作成功!'
        })
