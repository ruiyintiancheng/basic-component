<template>
    <div>
        <el-dialog :title="title" fullscreen append-to-body :close-on-click-modal='false' :visible.sync="formVisible" custom-class="dialog-default no-padding no-footer">
            <div class="dialog-contant-default" v-if="formVisible">
                <el-tabs v-model="activeName" class="conditionTabs">
                    <el-tab-pane label="计算流程图" name="first">
                      <div class="task-options">
                        <div class="btn-group">
                          <div class="btn btn-flush" @click="flashCanvas">
                            <el-button size="mini" plain>刷新</el-button>
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="ignore" /> 忽略条件
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="jump" /> 跳过
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="stopHandler" /> 中止
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="subTask" /> 子任务
                          </div>
                           <div class="btn img-btn">
                            <img class="btn mode" :src="reRun" /> 重算中
                          </div>
                           <div class="btn img-btn">
                            <img class="btn mode" :src="runShape" /> 计算中
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="noRecalculatedShape" /> 未重算
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="noCalculatedShape" /> 未计算
                          </div>
                           <div class="btn img-btn">
                            <img class="btn mode" :src="errorShape" /> 失败
                          </div>
                          <div class="btn img-btn">
                            <img class="btn mode" :src="successShape" /> 成功
                          </div>
                        ...
                        <div id="flowChart">
                            <ul class='contextmenu g6execute'>
                              ...
                              <li v-if="reShow" @mouseover="secondFloor=true" @mouseout="secondFloor=false">重算
                                <ul class='contextmenu g6execute second-floor' v-show="secondFloor">
                                  <li @click="recalculation(1)">当前节点</li>
                                  <li @click="recalculation(2)">所有下级</li>
                                  <li @click="recalculation(3)">当前及所有下级</li>
                                </ul>
                              </li>
                              <li v-if="handerShow" @mouseover="threeFloor=true" @mouseout="threeFloor=false">处理
                                <ul class='contextmenu g6execute second-floor' v-show="threeFloor">
                        ...
                        <flow-structure ref="flowStructure" @centeNode="centeNode" :nodes="nodes"></flow-structure>
                    </el-tab-pane>
                    <el-tab-pane label="计算流程列表" name="second">
                      <table-list :taskFlowId='taskFlowId' @searchNode='searchNode' @openNodeDialog='openNodeDialog'></table-list>
                    </el-tab-pane>
                </el-tabs>
            </div>
        </el-dialog>
        <el-dialog title="下级节点信息" append-to-body :close-on-click-modal='false' :visible.sync="nodeVisible" top="8vh" custom-class="dialog-default dialog-default-more">
            <div class="dialog-contant-default">
                <el-table :data="tableData" style="width: 100%">
                  <el-table-column type="index" width="50"></el-table-column>
                  <el-table-column prop="node_name" label="节点名称" ></el-table-column>
                  <el-table-column prop="field_convert_map.node_type" label="节点类型" ></el-table-column>
                  ...
                  <el-table-column label="是否重设子任务">
                    <template slot-scope="scope">
                        <el-select v-if="scope.row.sub_task_count !== 0" v-model="scope.row.reSub" placeholder="请选择">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="confirmRe()">确 定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
<script>
import noCalculatedShape from './flowPng/noCalculated.png' // 未计算
import errorShape from './flowPng/error.png' // 错误
import successShape from './flowPng/success.png' // 成功
import runShape from './flowPng/run.png' // 运行中
import reRun from './flowPng/reRun.png'
import noRecalculatedShape from './flowPng/noRecalculated.png' // 未重算
import subTask from './flowPng/subTask.png' // 子任务图标
import stopHandler from './flowPng/stopHandler.png' // 停止处理
import jump from './flowPng/jump.png' // 跳过处理
import ignore from './flowPng/ignore.png' // 忽略处理
import { registG6Shape } from './g6Shape'
