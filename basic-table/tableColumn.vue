    ...
    <div>
      <el-table-column  v-show="false" width="1">
        ...
        <el-table-column v-for="(column,index) in columns" :key="index"
         ...
         :column-key="paramString(column['column-key'])"
         :label="column.label"
         :width="paramString(column['width'])"
         :min-width="160"
         :fixed="column['fixed']==='true'?true:paramString(column['fixed'])"
         :render-header="paramObjectName(column['render-header'])"
         :sortable="paramString(column['sort-by'])?'custom':null"
         ...
         <template slot-scope="scope">
            <el-tooltip :disabled="textJug(columnData(scope.row,column.prop))"  :content="columnData(scope.row,column.prop)?columnData(scope.row,column.prop)+'':''" placement="top"  effect="light" :visible-arrow="false">
              <div class="basic-table-column">
                  {{columnData(scope.row,column.prop)}}
              </div>
            </el-tooltip>
         </template>
             <table-column v-if="column.children && column.children.length!==0" :columns="column.children" :otherTime="true"></table-column>
        </el-table-column>
